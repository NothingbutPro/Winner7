...
import android.ics.com.winner7.BuildConfig;
import android.ics.com.winner7.ProfileActivity;
...
public class CameraUtils {
    ...
    public static File getOutputMediaFile(int type) {
        ...
        File mediaStorageDir = new File(
                ...
                ProfileActivity.GALLERY_DIRECTORY_NAME);

        // Create the storage directory if it does not exist
        if (!mediaStorageDir.exists()) {
            if (!mediaStorageDir.mkdirs()) {
                Log.e(ProfileActivity.GALLERY_DIRECTORY_NAME, "Oops! Failed create "
                        + ProfileActivity.GALLERY_DIRECTORY_NAME+ " directory");
                return null;
            }
        }
        ...
        File mediaFile;
        if (type == ProfileActivity.MEDIA_TYPE_IMAGE) {
            mediaFile = new File(mediaStorageDir.getPath() + File.separator
                    + "IMG_" + timeStamp + "." + ProfileActivity.IMAGE_EXTENSION);
        }  else {
            return null;
        }
